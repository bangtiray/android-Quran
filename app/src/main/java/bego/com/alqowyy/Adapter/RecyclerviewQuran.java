...
public class RecyclerviewQuran extends RecyclerView.Adapter<RecyclerviewQuran.ViewHolder> {
    ...
    private ArrayList<ItemQuran> mData = new ArrayList<>();

    public RecyclerviewQuran(Context c) {
        this.c = c;
        inflater = (LayoutInflater) c.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    }

    public void addItem(ArrayList<ItemQuran> mData) {
        this.mData = mData;
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        holder.sura_title.setText(mData.get(position).getQuran());
        holder.sura_number.setText(mData.get(position).getAyat());
        holder.surah_id.setText(mData.get(position).getTerjemah());
